...
def chat():
    ...
    rooms.append(Room("Äijät", ''))
    rooms.append(Room("Automeisselöinnit", ''))
    rooms.append(Room("TKK", "aalto1"))
                    ...
                    if data:
                        ...
                        if msg[0] == '.': #alla olevat if lauseet toteuttavat käyttäjän komennot, esim ".room"
                            ...
                            if msg[:8] == 'roomlist':
                                ...
                                sock.send('\r[{:%Y-%m-%d %H:%M:%S}]'.format(datetime.datetime.now()) + ' [SERVER]: .room arg, .roomlist, .newroom nimi psw\n')
                                
                            elif msg[:6] == 'upload':
                                print "Käyttäjä lähettää tiedostoa: "
                                filename = msg[7:].rstrip()
                                print(filename) 
                                ...
                                upload(f)
                                    ...
                                    #clientbook[int(socketbook[sock])]
                                ...
                                sock.send('\r[{:%Y-%m-%d %H:%M:%S}]'.format(datetime.datetime.now()) + ' [SERVER]: Voi luoja... kirjoita .help vaikka...\n')
                        ...
                        print '[{:%Y-%m-%d %H:%M:%S}]'.format(datetime.datetime.now())+" [SERVER]: "+clientbook[int(socketbook[sock])]+" lähti, eikä enää ikinä tule takaisin"
                        roomcast(server_socket, server_socket, roombook[int(socketbook[sock])], clientbook[int(socketbook[sock])]+" lähti, eikä enää ikinä tule takaisin", socketbook, clientbook, roombook)
                        #broadcast(server_socket, sock, "Client (%s, %s) is offline\n" % clients[int(data.partition(';')[0].rstrip())])
...
def broadcast (server_socket, sendersock, message, socketbook, clientbook):
    ...
    for socket in SOCKET_LIST:
        if socket != server_socket and socket != sendersock:
            ...
            #except:
            #    socket.close()
            #    print "socketi poistui"
                #print '[{:%Y-%m-%d %H:%M:%S}]'.format(datetime.datetime.now())+" [Server]: "+clientbook[socketbook[sendersock]]+" lähti, eikä enää ikinä tule takaisin"
                #broadcast(server_socket, sock, ' lähti, eikä enää ikinä tule takaisin', socketbook, clientbook)
